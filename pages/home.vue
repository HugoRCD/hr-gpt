...
<template>
  <div class="md:pl-64 w-full">
    <div
      ...
      <div v-for="(person, personIdx) in personalities" :key="person.job"
        ...
        <div>
          ...
          <span :class="`${person.iconForeground}`">
            {{ person.name }}</span> - <span>{{ person.job }}
        </span>
        </div>
        <div>
        ...
        <span class="pointer-events-none absolute top-6 right-6 text-muted group-hover:text-gray-400"
              aria-hidden="true">
